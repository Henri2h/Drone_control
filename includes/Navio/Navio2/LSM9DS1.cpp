...
bool LSM9DS1::initialize()
{
    ...
    WriteReg(DEVICE_ACC_GYRO, LSM9DS1XG_CTRL_REG6_XL, BITS_ODR_XL_952HZ | BITS_FS_XL_8G); // acc scale
    usleep(200);
...
void LSM9DS1::update()
{
    ...
    // keep using degres
    _gx = ((float)bit_data[0] * gyro_scale);
    _gy = ((float)bit_data[1] * gyro_scale);
    _gz = ((float)bit_data[2] * gyro_scale);
