...
GPS::GPS()
{
	...
	ready = false;
	previousStatus_gps_fix = -2;
}

void GPS::initialize(double *status)
{
	this->status = status;
...
void GPS::update()
{
	...
	if (gps.decodeSingleMessage(Ublox::NAV_STATUS, pos_data) == 1)
	{
		...
		if (status[status_gps_fix] != previousStatus_gps_fix)
		{
			std::cout << "[ GPS ] : status gps fix : ";
			switch ((int)status[status_gps_fix])
			{
			case 0x00:
				printf("no fix\n");
				break;
